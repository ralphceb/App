...
var tokens;
  function handleClick()
  {
     if(document.getElementById('amountInput').value <= 0){
         alert("Invalid withdraw value");
     return false;
  }
     else{
    	 if(document.getElementById('amountInput').value > ${acount.founds}){
			alert("You don't have money, go to deposits ");
        	 return false
		...
		<table class="table table-striped">
			...
			<thead>
				<tr>
					...
					<th style="text-decoration:underline;font-size: larger;">2 WITHDRAW DETAILS</th>
					<th>3 CONFIRMATION</th>
				</tr>
			</thead>		
		</table>
		
		<form:form method="post" commandName="withdrawD" onsubmit="return handleClick();">
		<fieldset class="form-group">
			<form:label path="acount">From this Acount:</form:label>
			<form:label path="acount" style="text-decoration:underline;font-size: larger;"> ${acount.acount}</form:label>
			<form:errors path="acount" cssClass="text-warning" />
		</fieldset>
		
		<fieldset class="form-group">
			<form:label path="founds">Amount:</form:label>
			<form:input id="amountInput" path="founds" type="text" class="form-control"
				required="required" />
